...
def balanced(m):
    ...
    True
    """
    assert is_mobile(m), "must call on a mobile."
    left_arm, right_arm = left(m), right(m)
    if not torque(left_arm) == torque(right_arm):
        return False
    left_balance, right_balance = True, True
    if is_mobile(end(left_arm)):
        left_balance = True if balanced(end(left_arm)) else False
    if is_mobile(end(right_arm)):
        right_balance = True if balanced(end(right_arm)) else False
    return left_balance and right_balance


def torque(a):
    assert is_arm(a), "must call on a arm"
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    assert upper_bound(y) > lower_bound(y) and upper_bound(y) * lower_bound(y) > 0
