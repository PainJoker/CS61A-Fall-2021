...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt does no action but has large HP."""
...
class HungryAnt(Ant):
    ...
    After eating a Bee, a HungryAnt must spend 3 turns chewing before eating again. 
    If there is no bee available to eat, HungryAnt will do nothing."""
...
class TankAnt(ContainerAnt):
    """ The TankAnt is a container that protects an ant in its place 
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while back_place.exit is not None:
            if back_place.ant:
                back_place.ant.buff()
                if back_place.ant.is_container and back_place.ant.ant_contained:
                    back_place.ant.ant_contained.buff()
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.slow_countdown > 0 and gamestate.time % 2 == 1:
            ...
            elif self.health > 0 and self.scared_countdown > 0:
                if entrance.is_hive: pass
                else: 
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        # BEGIN Problem Optional 2
        ret_dict, front_place, distance = {}, self.place, 0
        while not front_place.is_hive:
            for bee in front_place.bees:
                ret_dict[bee] = distance
            if front_place.ant:
                alley = front_place.ant
                if distance == 0 and alley.is_container:
                    ret_dict[alley] = distance
                elif alley.is_container:
                    ret_dict[alley] = distance
                    ret_dict[alley.contained_ant] = distance
                elif alley is not self:
                    ret_dict[alley] = distance
            front_place, distance = front_place.entrance, distance + 1
        return ret_dict
        # END Problem Optional 2
